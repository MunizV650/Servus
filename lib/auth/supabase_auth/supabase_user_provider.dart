...
class ServusSupabaseUser extends BaseAuthUser {
  ServusSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> servusSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ServusSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
